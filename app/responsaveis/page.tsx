...
export default function ResponsaveisObrasPage() {
  ...
  function openNewResponsible() {
    if (responsibles.length >= limits.responsavelLimit) {
      setNotice(
        `O plano ${limits.label} permite at├® ${limits.responsavelLimit} respons├íveis.`
      );
      return;
    }
    setEditingId(null);
  ...
  async function saveResponsible(event: FormEvent<HTMLFormElement>) {
    ...
    if (projectAlreadyLinked) {
      setNotice("Esta obra j├í possui um respons├ível principal.");
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        setNotice("Respons├ível atualizado com sucesso.");
      } else {
        ...
        setNotice("Respons├ível adicionado com sucesso.");
      }
      setModalOpen(false);
      setEditingId(null);
      setForm(emptyForm);
  ...
  return (
    <AppShell
      title="Respons├íveis pelas Obras"
      subtitle="Cadastre e vincule um respons├ível principal para cada obra."
      action={
        <button
          type="button"
    ...
    >
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          ...
          value={`${limits.responsavelLimit} respons├íveis permitidos`}
          helper="1 respons├ível por obra"
        />
        <Metric
          label="Respons├íveis cadastrados"
          value={`${responsibles.length}/${limits.responsavelLimit}`}
          helper="Principais e ativos"
        />
        <Metric
          label="Obras com respons├ível"
          value={`${responsibles.length}/${obras.length}`}
          helper={`${responsibles.length} de ${obras.length} obras possuem respons├ível`}
        />
        <Metric
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-black text-foundation">
                  Respons├íveis cadastrados
                </h2>
              </div>
              <p className="mt-2 text-sm font-semibold text-graphite/60">
                Cada obra possui apenas um respons├ível principal.
              </p>
            </div>
            <p className="text-xs font-bold text-graphite/50">
              Gratuito: 1 ┬À Mensal: 3 ┬À Premium: 10
            </p>
          </div>
            ...
            <table className="w-full border-separate border-spacing-y-2 text-left">
              <thead>
                <tr className="text-xs font-black uppercase text-graphite/48">
                  <th className="px-3 pb-1">Respons├ível</th>
                  <th className="px-3 pb-1">Fun├º├úo</th>
                  <th className="px-3 pb-1">Obra vinculada</th>
                  <th className="px-3 pb-1">WhatsApp</th>
                  <th className="px-3 pb-1">Status</th>
                  <th className="px-3 pb-1 text-right">A├º├Áes</th>
                </tr>
              </thead>
              <tbody>
          ...
          {responsibles.length === 0 ? (
            <div className="mt-5 rounded-[8px] bg-concrete p-6 text-center">
              ...
              <h3 className="mt-3 font-black text-foundation">
                Nenhum respons├ível cadastrado
              </h3>
              <p className="mt-1 text-sm font-semibold text-graphite/60">
                Adicione o respons├ível principal da sua primeira obra.
              </p>
            </div>
          ) : null}
        </Card>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm font-semibold text-graphite/60">
                  Vincule uma pessoa como respons├ível principal de uma obra.
                </p>
              </div>
              <button
                type="button"
